...
class DialogMetrics(object):
    ...
    @property
    def action_train_accuracy(self):
        return self.n_train_corr_actions / max(1., self.n_examples)

    @property
    def action_train_d_accuracy(self):
        return self.n_train_corr_dialog_actions / max(1., self.n_dialogs)
    ...
    def action_train_weighted_f_beta(self, beta=1):
        ...
        return np.sum(weights * self.action_train_fs_beta(beta=beta))

    @property
    def train_accuracy(self):
        return self.n_train_corr_examples / max(1., self.n_examples)

    @property
    def train_d_accuracy(self):
        return self.n_train_corr_dialogs / max(1., self.n_dialogs)
