...
class DSTC2Teacher(Teacher):
    """A dstc2 teacher class for doing dialog with known response actions.
    """
    ...
    def load_cands(self, path):
        """Load global fixed set of templates that the teacher provides
        ...
        with open(path) as read:
            for line in read:
                line = line.strip()
                if line:
                    # if is a json, then label_candidates are all the replies
                    if line.startswith('{') and not cands_are_replies:
